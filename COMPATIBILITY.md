...
This repository preserves the upstream history and license of SWATdoctR. Package names remain unchanged so existing R scripts can continue using `library(SWATdoctR)`. This is a tested development update, not an upstream release.

Upstream starting commit: `4b1aee0988e63f9678b23cd6e648f89bce34ef14`. SWATrunR uses upstream `remove_legacy` (1.1.0.9017), matching the bundled workflow; its one local difference was portable library propagation to workers, which is retained in this update.
